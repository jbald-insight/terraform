...
You can use the optional `out=FILE` option with `> FILE.EXT -no-color` to export a human-readable plan, which allows users to read the plan that has been created.  This output is necessary for users to read and understand the plan that machine-readable plan which `terraform plan` produces. 
